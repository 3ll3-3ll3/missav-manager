...
test("生产 API 拒绝缺少 ChatGPT 身份头，开发主机仍可本地验收", () => {
  assert.throws(
    () =>
      apiResponse.requireAuthenticated(
        new Request("https://private.example/api/records"),
      ),
    /ChatGPT/,
  );
  assert.doesNotThrow(() =>
...
test("数据库迁移覆盖完整业务语义、唯一约束与分块校验表", async () => {
  ...
  const compatibilityMigration = await readFile(
    projectFile("drizzle/0004_classy_pixie.sql"),
    "utf8",
  );
  const workflowMigration = await readFile(
    projectFile("drizzle/0005_workflow_contract.sql"),
    "utf8",
  );
  ...
  assert.match(sql, /import_batch_chunks_batch_index_uq/);
  assert.match(sql, /telegram_tool_queue_message_tool_uq/);
  assert.match(sql, /ON DELETE cascade/);
  assert.match(sql, /encrypted_session/);
  assert.match(sql, /telegram_auth_flows_expires_idx/);
  assert.match(compatibilityMigration, /valid no-op/);
  assert.match(compatibilityMigration, /SELECT 1/);
  for (const column of [
    "status",
    "stats_json",
    "options_json",
    ...
    assert.match(workflowMigration, new RegExp(column));
  for (const index of [
    "telegram_messages_source_date_id_idx",
    "telegram_tool_queue_message_status_idx",
    "telegram_tool_queue_tool_status_date_id_idx",
  ])
    assert.match(workflowMigration, new RegExp(index));
  assert.match(workflowMigration, /UPDATE `task_inbox` SET `phase`=CASE/);
  for (const column of [
    "sync_cursor_message_id",
    "sync_target_message_id",
    "encrypted_challenge",
    "content_hash",
    "remote_edited_at",
    "remote_deleted_at",
...
test("Secret 变量只在服务端读取，客户端与默认配置不含 Token 值", async () => {
  const botServer = await readFile(
    projectFile("lib/server-telegram.ts"),
    "utf8",
  );
  const mtServer = await readFile(projectFile("lib/server-mtproto.ts"), "utf8");
  ...
  assert.match(botServer, /TELEGRAM_BOT_TOKEN/);
  assert.match(mtServer, /TELEGRAM_API_ID/);
  assert.match(mtServer, /TELEGRAM_SESSION_ENCRYPTION_KEY/);
  ...
  for (const secretName of [
    ...
    assert.doesNotMatch(settings, new RegExp(secretName));
  assert.doesNotMatch(botServer, /\d{8,12}:[A-Za-z0-9_-]{20,}/);
  assert.doesNotMatch(mtServer, /console\.(log|error)|process\.env/);
});
...
test("Telegram 配置读取期间不再伪装成未配置状态", async () => {
  ...
  const telegramRoute = await readFile(
    projectFile("app/api/telegram/route.ts"),
    "utf8",
  );
  const mtprotoRoute = await readFile(
    projectFile("app/api/telegram/mtproto/route.ts"),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    settings,
    /Promise\.all\(\[loadHub\(\), loadMtproto\(\)\]\)/,
  );
...
test("Worker 冷启动优先执行只读 schema 探针，不重复争抢 D1 写锁", async () => {
  ...
  const compatibilityIndex = store.indexOf(
    "await applyTelegramHubSchemaCompatibility(db)",
  );
  const ddlIndex = store.indexOf("const statements = [");
  assert.ok(
    probeIndex > 0 && ddlIndex > probeIndex,
    "read-only schema probe must precede legacy DDL fallback",
  );
  assert.ok(
    compatibilityIndex > probeIndex && ddlIndex > compatibilityIndex,
    "targeted Telegram schema repair must precede legacy DDL fallback",
  );
...
test("产品状态不再把 Telegram 个人 API 宣称为 Windows 独占", async () => {
  ...
  const telegramPanel = await readFile(
    projectFile("app/components/telegram-panel.tsx"),
    "utf8",
  );
  const settings = await readFile(
  ...
  assert.doesNotMatch(
    workbench,
    /Telegram 个人账号 API、扫码\/手机号登录、远端标已读。/,
  );
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(telegramPanel, new RegExp(forbidden));
});
...
test("MTProto 登录 API 覆盖二维码、手机号、验证码、2FA、恢复、取消与注销", async () => {
  ...
  const socket = await readFile(
    projectFile("lib/cloudflare-telegram-socket.ts"),
    "utf8",
  );
  for (const action of [
    "start-phone",
...
test("MTProto 服务端不持久化手机号、验证码、密码或二维码内容", async () => {
  ...
  const schema = await readFile(
    projectFile("drizzle/0002_small_garia.sql"),
    "utf8",
  );
  for (const column of ["phone", "code", "password", "qr_url", "qr_token"])
    assert.doesNotMatch(schema, new RegExp("`" + column + "`", "i"));
...
test("MTProto 平台探针不读取凭据且只使用固定 Telegram 端点", async () => {
  const probe = await readFile(
    projectFile("lib/server-mtproto-probe.ts"),
    "utf8",
  );
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(probe, new RegExp(forbidden.replace(".", "\\."), "i"));
});
