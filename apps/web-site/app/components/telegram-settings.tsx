...
type Hub = { configured: boolean; connections: Connection[]; sources: Source[]; bindings: Binding[]; syncRuns: Array<Record<string, unknown>>; readStates: Array<Record<string, unknown>> };
type MtprotoStatus = { apiConfigured: boolean; encryptionConfigured: boolean; authorized: boolean; accountLabel: string; stage: "idle" | "waiting_qr" | "waiting_code" | "waiting_password" | "authorized"; mode: "" | "qr" | "phone"; expiresAt: string };
const EMPTY_MTPROTO: MtprotoStatus = { apiConfigured: false, encryptionConfigured: false, authorized: false, accountLabel: "", stage: "idle", mode: "", expiresAt: "" };
...
export default function TelegramSettingsPanel() {
  ...
  const [hub, setHub] = useState<Hub>({ configured: false, connections: [], sources: [], bindings: [], syncRuns: [], readStates: [] });
  ...
  const [readMaxId, setReadMaxId] = useState("");

  const load = useCallback(async () => {
    try {
      const [nextHub, nextMtproto] = await Promise.all([api("/api/telegram?view=settings"), api("/api/telegram/mtproto")]);
      setHub(nextHub);
      setMtproto(nextMtproto);
    ...
    } catch (error) { setNotice(error instanceof Error ? error.message : "Telegram 设置加载失败"); }
  }, []);
  ...
  async function runAction(action: string, values: Record<string, unknown> = {}) {
    ...
    try {
      ...
      await load();
    } catch (error) { setNotice(error instanceof Error ? error.message : "Telegram 登录操作失败"); }
  ...
  async function markRead(sourceId: string) {
    if (!readMaxId.trim()) { setNotice("请先填写要标记的 max_id"); return; }
    setBusy(true);
    try { await api("/api/telegram", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "mark-read", sourceId, maxId: readMaxId }) }); setNotice("已按来源和 max_id 执行手动已读"); await load(); }
  ...
  return <div className="stack-lg telegram-settings">
    ...
    {tab === "connections" && <div className="stack-md">
      <section className="card"><div className="section-heading"><div><span className="eyebrow">全局连接</span><h3>Bot API</h3></div><span className={`batch-status ${hub.configured ? "applied" : ""}`}>{hub.configured ? "Secret 已配置" : "未配置"}</span></div><p>Bot 只在这里配置。网站只允许这一处调用全局 <code>getUpdates</code>，五个工具不会争抢游标。</p><div className="button-row"><button className="primary" disabled={!hub.configured || busy} onClick={pullBot}>手动接收 Bot 更新</button><button onClick={() => setTab("sources")}>查看全局会话库</button></div></section>
      <section className="card"><div className="section-heading"><div><span className="eyebrow">全局连接</span><h3>{mtproto.authorized ? "个人账号已连接" : "个人账号 MTProto"}</h3></div><span className={`batch-status ${mtproto.authorized ? "applied" : ""}`}>{mtproto.authorized ? "已连接" : "未完成登录"}</span></div><p>二维码、手机号、验证码、两步验证和加密 Session 只在这个全局连接中心出现；工具页面不会要求重新登录。</p><div className="mtproto-config-row"><span className={`batch-status ${mtproto.apiConfigured ? "applied" : ""}`}>API {mtproto.apiConfigured ? "已配置" : "未配置"}</span><span className={`batch-status ${mtproto.encryptionConfigured ? "applied" : ""}`}>Session 加密 {mtproto.encryptionConfigured ? "已配置" : "未配置"}</span></div>
      ...
      <section className="card"><span className="eyebrow">连接状态</span><div className="connection-grid">{hub.connections.map((connection) => <article key={connection.connection_id} className="connection-card"><div><strong>{connection.label || connection.connection_id}</strong><small>{connection.kind === "personal" ? "个人 API" : connection.kind === "bot" ? "Bot API" : "文件导入"} · {connection.network_status || "未知网络"}</small></div><span className={`batch-status ${connection.status === "connected" ? "applied" : ""}`}>{connection.status}</span>{connection.last_error && <p className="error-text">{connection.last_error}</p>}</article>)}</div></section>
    </div>}
    ...
    {tab === "sources" && <div className="stack-md"><section className="card"><div className="section-heading"><div><span className="eyebrow">全局来源</span><h3>{hub.sources.length.toLocaleString()} 个群组 / 频道</h3></div><span className="subtle">最多按 100 个来源设计</span></div><div className="button-row"><button className="primary" disabled={busy || !mtproto.authorized} onClick={discover}>刷新个人账号会话</button><label className="file-button"><input type="file" accept=".json" onChange={importJson} />导入 Telegram 官方 JSON</label><button onClick={() => setSelectedSources(new Set(filteredSources.map((source) => source.id)))}>全选当前筛选</button><button onClick={() => setSelectedSources(new Set())}>清空选择</button></div><div className="source-toolbar"><input value={sourceSearch} onChange={(event) => setSourceSearch(event.target.value)} placeholder="搜索名称、用户名、Telegram ID" /><select value={sourceType} onChange={(event) => setSourceType(event.target.value)}><option value="">全部类型</option><option value="group">普通群</option><option value="supergroup">超级群</option><option value="channel">广播频道</option></select><input value={readMaxId} onChange={(event) => setReadMaxId(event.target.value)} placeholder="手动已读 max_id" inputMode="numeric" /></div><div className="source-action-row"><select value={syncMode} onChange={(event) => setSyncMode(event.target.value as typeof syncMode)}><option value="incremental">从最新位置增量</option><option value="recent">最近 N 条</option><option value="range">指定时间范围</option><option value="history">从本地最早处继续</option></select><input type="number" min="1" max="20000" value={syncLimit} onChange={(event) => setSyncLimit(event.target.value)} /><input type="datetime-local" value={syncStart} onChange={(event) => setSyncStart(event.target.value)} /><input type="datetime-local" value={syncEnd} onChange={(event) => setSyncEnd(event.target.value)} /><button disabled={busy || !selectedSources.size || !mtproto.authorized} onClick={syncPersonal}>同步所选个人来源</button></div></section><section className="card source-table-wrap"><div className="source-table desktop-only"><div className="source-table-head"><span>会话</span><span>类型 / ID</span><span>连接</span><span>本地状态</span><span>已读策略</span><span>手动已读</span><span /></div>{filteredSources.map((source) => <div className={`source-table-row ${selectedSources.has(source.id) ? "selected" : ""}`} key={source.id}><label><input type="checkbox" checked={selectedSources.has(source.id)} onChange={() => toggleSource(source.id)} /><strong>{source.name}</strong><small>{source.username ? `@${source.username.replace(/^@/, "")}` : "无用户名"}</small></label><span>{source.chat_type || "未知"}<small>{source.external_chat_id}</small></span><span>{source.connection_id === "telegram-personal" ? "个人 API" : source.connection_id === "telegram-bot" ? "Bot" : "导入"}</span><span>{Number(source.message_count || 0).toLocaleString()} 条<small>待处理 {Number(source.pending_count || 0).toLocaleString()} · {source.access_status}</small></span><select value={String(hub.readStates.find((row) => String(row.source_id) === source.id)?.policy || "never")} onChange={(event) => updateSource(source.id, { readPolicy: event.target.value })}><option value="safe_auto">safe_auto</option><option value="never">never</option><option value="manual">manual</option></select><button disabled={busy || source.connection_id !== "telegram-personal"} onClick={() => markRead(source.id)}>标记</button><button onClick={() => updateSource(source.id, { archived: !Boolean(source.archived) })}>{source.archived ? "重新启用" : "停用"}</button></div>)}{!filteredSources.length && <div className="empty compact"><strong>没有符合条件的会话</strong><p>先完成全局连接、刷新会话或导入官方 JSON。</p></div>}</div><div className="mobile-source-list mobile-only">{filteredSources.map((source) => <label key={source.id} className="mobile-source-card"><input type="checkbox" checked={selectedSources.has(source.id)} onChange={() => toggleSource(source.id)} /><strong>{source.name}</strong><small>{source.chat_type} · {source.external_chat_id} · {source.connection_id}</small><small>{Number(source.message_count || 0).toLocaleString()} 条 · 待处理 {Number(source.pending_count || 0).toLocaleString()}</small></label>)}</div></section></div>}
