...
export default function ResultTable({
  ...
  const [columnOrder, setColumnOrder] = useState<Column[]>(COLUMNS.map((column) => column.key));
  const [columnWidths, setColumnWidths] = useState<Record<Column, number>>({ primaryValue: 260, secondaryValue: 320, status: 140, tags: 220, source: 220 });
  ...
  const visibleColumns = columnOrder.map((key) => COLUMNS.find((column) => column.key === key)!).filter((column) => !hidden.has(column.key));
  ...
  function removeSelected() {
    ...
    if (!window.confirm(`从当前草稿移除 ${selected.size} 条结果？已保存的原历史不会被删除，可随时重新载入。`)) return;
  ...
  async function copy() {
    ...
    try { await navigator.clipboard.writeText(text(scope)); notice(`已复制 ${scope.length.toLocaleString()} 条结果`); }
    catch { notice("浏览器拒绝访问剪贴板；请使用上方纯文本输出框手动全选复制"); }
  }

  function scopeIds() {
    return selected.size ? new Set(selected) : new Set(filtered.map((row) => row.resultKey));
  }

  function replaceInScope() {
    if (!findText) { notice("请输入要查找的文本"); return; }
    const ids = scopeIds();
    if (!ids.size) return;
    const label = COLUMNS.find((column) => column.key === replaceColumn)?.label || replaceColumn;
    if (!window.confirm(`在${selected.size ? `所选 ${ids.size}` : `当前筛选 ${ids.size}`}条结果的“${label}”中执行查找替换？原历史仍保留，可返回历史恢复。`)) return;
    let changes = 0;
    const nextRows = results.map((row) => {
      ...
      changes += 1;
      return replaceColumn === "tags" ? { ...row, tags: after.split(/[|,，\r\n]+/).map((item) => item.trim()).filter(Boolean) } : { ...row, [replaceColumn]: after };
    });
    setResults(nextRows);
    notice(`已在草稿中替换 ${changes.toLocaleString()} 行；保存修改后生成新历史，原历史不变`);
  }

  function batchModify() {
    const ids = scopeIds();
    if (!ids.size || !batchValue.trim()) { notice("请选择字段并填写批量值"); return; }
    const label = COLUMNS.find((column) => column.key === batchColumn)?.label || batchColumn;
    if (!window.confirm(`把${selected.size ? `所选 ${ids.size}` : `当前筛选 ${ids.size}`}条结果的“${label}”批量修改？原历史仍保留。`)) return;
    setResults((current) => current.map((row) => ids.has(row.resultKey)
      ? batchColumn === "tags"
        ? { ...row, tags: batchValue.split(/[|,，\r\n]+/).map((item) => item.trim()).filter(Boolean) }
    ...
    try {
      ...
      for (let rowOffset = 0; rowOffset < matrix.length; rowOffset += 1) {
        ...
        for (let columnOffset = 0; columnOffset < matrix[rowOffset].length; columnOffset += 1) {
          ...
          if (column.key === "tags") next = { ...next, tags: nextValue.split(/[|,，]/).map((item) => item.trim()).filter(Boolean) };
          else next = { ...next, [column.key]: nextValue };
        }
        updates.set(target.resultKey, next);
      }
      setResults((current) => current.map((row) => updates.get(row.resultKey) || row));
      notice(`已从剪贴板粘贴 ${updates.size.toLocaleString()} 行；保存历史后持久化`);
    } catch {
  ...
  function keyboard(event: React.KeyboardEvent<HTMLDivElement>) {
    ...
    else if (modifier && event.key.toLowerCase() === "v") { event.preventDefault(); void pasteRange(); }
  ...
  return (
    <div className={`stack-md result-table-workspace ${expanded ? "expanded" : ""}`} tabIndex={0} onKeyDown={keyboard}>
      <section className="data-toolbar card"><div className="searchbox"><span>⌕</span><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); setSelected(new Set()); }} placeholder="搜索主值、链接、状态或标签" /></div><select value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); setSelected(new Set()); }}><option value="">全部状态</option>{statuses.map((item) => <option key={item}>{item}</option>)}</select><button onClick={selectPage}>全选本页</button><button onClick={() => setSelected(new Set(filtered.map((row) => row.resultKey)))}>全选筛选结果</button><button onClick={() => setColumnsOpen((value) => !value)}>列设置</button><button onClick={() => setExpanded((value) => !value)}>{expanded ? "退出展开" : "展开表格"}</button></section>
      {columnsOpen && <section className="card result-columns"><strong>列显示、顺序与宽度</strong>{columnOrder.map((key) => { const column = COLUMNS.find((item) => item.key === key)!; return <div className="result-column-setting" key={column.key}><label><input type="checkbox" checked={!hidden.has(column.key)} onChange={(event) => setHidden((current) => { const next = new Set(current); if (event.target.checked) next.delete(column.key); else next.add(column.key); return next; })} />{column.label}</label><input aria-label={`${column.label}列宽`} type="range" min="100" max="600" step="20" value={columnWidths[column.key]} onChange={(event) => setColumnWidths((current) => ({ ...current, [column.key]: Number(event.target.value) }))} /><span>{columnWidths[column.key]} px</span><button onClick={() => moveColumn(column.key, -1)}>←</button><button onClick={() => moveColumn(column.key, 1)}>→</button></div>; })}</section>}
      <details className="card find-replace-panel"><summary>查找替换与批量修改</summary><div className="find-replace-grid"><select value={replaceColumn} onChange={(event) => setReplaceColumn(event.target.value as Column)}>{COLUMNS.map((column) => <option key={column.key} value={column.key}>{column.label}</option>)}</select><input value={findText} onChange={(event) => setFindText(event.target.value)} placeholder="查找文本" /><input value={replaceText} onChange={(event) => setReplaceText(event.target.value)} placeholder="替换为（可留空）" /><button onClick={replaceInScope}>按当前范围替换</button><select value={batchColumn} onChange={(event) => setBatchColumn(event.target.value as Column)}>{COLUMNS.filter((column) => column.key !== "primaryValue").map((column) => <option key={column.key} value={column.key}>批量：{column.label}</option>)}</select><input value={batchValue} onChange={(event) => setBatchValue(event.target.value)} placeholder="批量修改值" /><button onClick={batchModify}>按当前范围批量修改</button></div><p className="hint">有选择时作用于所选；没有选择时作用于当前筛选。修改只进入草稿，保存后生成新历史，原历史保留。</p></details>
