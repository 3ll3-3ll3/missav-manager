...
export default function TelegramPanel({
  ...
  const visibleSources = useMemo(() => { const needle = sourceSearch.trim().toLowerCase(); return needle ? sources.filter((source) => [source.name, source.external_chat_id, source.chat_type].some((value) => String(value || "").toLowerCase().includes(needle))) : sources; }, [sources, sourceSearch]);
  ...
  async function refreshSyncMessages() {
    ...
    try {
      ...
      setNotice(`刷新同步完成：个人扫描 ${Number(personal.scanned || 0)}，Bot 新增 ${Number(bot.inserted || 0)}，编辑 ${Number((personal.edited || 0) + (bot.edited || 0))}，删除 ${Number((personal.deleted || 0) + (bot.deleted || 0))}；当前筛选匹配 ${candidateTotal} 条，隐藏无候选噪声 ${hidden} 条；复用全局连接、Bot offset、统一消息池与来源检查点`);
      setError(null);
    } catch (reason) {
  ...
  async function rerunErrors() {
    if (!selectedSources.size) return;
    await processPayload({ mode: "all", excludeIds: [], status: "error", search, start, end, sourceIds: [...selectedSources] }, "异常消息重跑完成");
  }
  ...
  return (
    <div className="telegram-workspace stack-md" tabIndex={0} onKeyDown={keyboard}>
      ...
      <section className="card"><div className="section-heading"><div><span className="eyebrow">Telegram 消息输入</span><h3>本次来源：已选 {selectedSources.size} / 已绑定 {sources.length}</h3></div><button onClick={onOpenTelegramSettings}>管理绑定</button></div>{!sources.length ? <div className="empty compact"><strong>当前工具还没有绑定来源</strong><p>先到全局中心为 {TOOL_LABELS[tool]} 绑定来源。</p><button className="primary" onClick={onOpenTelegramSettings}>打开绑定编辑器</button></div> : <details className="tool-source-disclosure"><summary><span><strong>选择本次同步的群组 / 频道</strong><small>默认收起；展开后可搜索并勾选，不会改变永久绑定</small></span><span>{selectedSources.size} 个已选</span></summary><div className="tool-source-picker-body"><div className="searchbox"><span>⌕</span><input value={sourceSearch} onChange={(event) => setSourceSearch(event.target.value)} placeholder="搜索群组名称、类型或 Telegram ID" /></div><div className="telegram-source-picker">{visibleSources.map((source) => <label key={source.id} className={selectedSources.has(source.id) ? "selected" : ""}><input type="checkbox" checked={selectedSources.has(source.id)} onChange={() => toggleSource(source.id)} /><span><strong>{source.name}</strong><small>{source.connection_id === "telegram-personal" ? "个人 API" : source.connection_id === "telegram-bot" ? "Bot" : "导入"} · {source.chat_type || "会话"} · {source.external_chat_id}</small></span><small>当前工具已绑定</small></label>)}</div><div className="button-row"><button onClick={() => setSelectedSources(new Set(visibleSources.map((source) => source.id)))}>全选搜索结果</button><button onClick={() => setSelectedSources(new Set())}>清空本次选择</button></div></div></details>}</section>
      ...
      <section className="table-card telegram-table-card">
        <div className="table-meta">
          ...
          <div className="button-row"><button disabled={busy || !selectedSources.size} onClick={() => void rerunErrors()}>重跑异常</button><button className="primary" disabled={busy || !selectedSources.size} onClick={() => void processAllPending()}>{activity === "process" ? "处理中…" : "一键处理所有未处理"}</button></div>
        </div>
        ...
        {!rows.length && <div className="empty compact"><strong>当前筛选没有可处理消息</strong><p>已处理、正文为空或无法被当前工具严格规则识别的内容不会进入工作列表。若刚同步过旧数据，请改用“回拉最近 N 条”重新读取富文本链接。</p>{hiddenNoiseCount > 0 && <small>本次已隐藏 {hiddenNoiseCount.toLocaleString()} 条无候选噪声。</small>}</div>}
