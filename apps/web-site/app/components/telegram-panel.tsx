...
export default function TelegramPanel({
  ...
  const [loadMode, setLoadMode] = useState<
    "cached" | "recent" | "range" | "history" | "incremental"
  >("incremental");
  ...
  function selectRow(index: number, event: React.MouseEvent) {
    if (rows[index].status === "deleted") return;
  ...
  async function refreshSyncMessages() {
    ...
    setOperation({
      ...
      status: "running",
      phase: "syncing",
      label: `正在同步 ${selectedSources.size} 个已绑定来源`,
      current: 0,
    ...
    try {
      ...
      const networkMs = Number(botTimings.remoteMs || 0);
      const databaseMs =
        Number(botTimings.databaseMs || 0) +
        Number(personalTimings.totalMs || 0);
      const totalMs =
        Number(botTimings.totalMs || 0) + Number(personalTimings.totalMs || 0);
      setOperationDetail(
        `Bot ${Number(bot.pages || 0)} 页 · Telegram 网络 ${networkMs.toFixed(0)} ms · 本地入库 ${databaseMs.toFixed(0)} ms · 总计 ${totalMs.toFixed(0)} ms`,
      );
      setNotice(
        `刷新同步完成：个人扫描 ${Number(personal.scanned || 0)}，Bot ${Number(bot.pages || 0)} 页 / 新增 ${Number(bot.inserted || 0)}，编辑 ${Number(personal.edited || 0) + Number(bot.edited || 0)}，删除 ${Number(personal.deleted || 0) + Number(bot.deleted || 0)}；网络 ${networkMs.toFixed(0)} ms，本地处理 ${databaseMs.toFixed(0)} ms；当前筛选匹配 ${candidateTotal} 条，隐藏无候选噪声 ${hidden} 条；复用全局连接、Bot offset、统一消息池与来源检查点`,
      );
      setError(null);
    } catch (reason) {
  ...
  return (
    ...
    >
      <section className="callout telegram-layer-callout">
        ...
        <p>
          这里只选择已绑定来源；登录、Bot offset、消息池和来源检查点仍由全局
          Telegram 中心唯一维护。
        </p>
      </section>
      <section className="card">
        ...
        {!sources.length ? (
          <div className="empty compact">
            ...
            <button className="primary" onClick={onOpenTelegramSettings}>
              打开绑定编辑器
            </button>
          </div>
        ) : (
      ...
      <section className="card">
        ...
        <div className="telegram-filters">
          <select
            value={loadMode}
            onChange={(event) =>
              setLoadMode(event.target.value as typeof loadMode)
            }
          >
            <option value="incremental">从检查点增量</option>
            <option value="recent">回拉最近 N 条</option>
            <option value="range">指定时间范围</option>
            <option value="history">从本地最早处继续</option>
            <option value="cached">只刷新本地消息池</option>
          </select>
          <input
            type="number"
            min="1"
            ...
            value={syncLimit}
            onChange={(event) =>
              setSyncLimit(Number(event.target.value) || 200)
            }
          />
          <input
            type="datetime-local"
          ...
          <button
            className="primary"
            disabled={busy || !selectedSources.size}
            onClick={() => void refreshSyncMessages()}
          >
            {activity === "sync" ? "同步中…" : "刷新同步消息"}
          </button>
        </div>
        <p className="subtle">
          个人账号只读取本次选择；Bot 继续消费唯一全局 getUpdates offset
          并进入统一消息池，当前工具视图只显示所选已绑定来源，不会创建第二套登录或游标。
        </p>
      </section>
      {operation && (
        <section
      ...
      <section className="data-toolbar card">
        ...
        <button onClick={() => void loadQueue()}>刷新列表</button>
      </section>
      {columnsOpen && (
        ...
        >
          ...
          <div className="bulkbar-group">
            ...
            <div className="bulkbar-buttons">
              ...
              <button disabled={busy} onClick={() => void exportChosen("txt")}>
                导出 TXT
              </button>
              <button disabled={busy} onClick={() => void exportChosen("csv")}>
                导出 CSV
              </button>
            </div>
          </div>
      ...
      <section className="table-card telegram-table-card">
        ...
        <div className="mobile-telegram-list mobile-only">
          {rows.map((row, index) => (
            ...
            >
              ...
              <strong>
                {row.source_name} · #{row.message_id}
              </strong>
              <p>{row.body}</p>
              <small>
                {row.candidate_count
                  ? `${row.candidate_count} 条结果`
                  : "尚无结果"}
              </small>
              <button
                onClick={(event) => {
        ...
        {!rows.length && (
          <div className="empty compact">
            ...
            <p>
              已处理、正文为空或无法被当前工具严格规则识别的内容不会进入工作列表。若刚同步过旧数据，请改用“回拉最近
              N 条”重新读取富文本链接。
            </p>
            {hiddenNoiseCount > 0 && (
              <small>
