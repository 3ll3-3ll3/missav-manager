...
export default function TelegramPanel({
  ...
  async function processPayload(payload: Record<string, unknown>, label: string) {
    setBusy(true);
    try {
      const result = await api("/api/telegram", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "process", tool, ...payload, deleteBody: true }) });
      setNotice(`${label}：处理 ${result.selected} 条，提取 ${result.resultCount} 条；恢复点 ${String(result.snapshotId || "").slice(0, 8)}… 已建立`);
      resetSelection();
      await loadQueue();
      onProcessed();
    } catch (reason) { setError(toUiError(reason, "Telegram 消息处理失败")); }
    finally { setBusy(false); }
  }
  ...
  return (
    <div className="telegram-workspace stack-md" tabIndex={0} onKeyDown={keyboard}>
      ...
      {selectedCount > 0 && <section className="bulkbar"><strong>已选 {selectedCount.toLocaleString()} 条</strong><button className="primary" disabled={busy} onClick={() => void processPayload(selectionPayload, "所选消息完成")}>处理所选</button><button disabled={busy} onClick={() => void queueStatus("ignored")}>忽略</button><button disabled={busy} onClick={() => void queueStatus("pending")}>恢复待处理</button><button onClick={() => void exportChosen("txt", true)}>复制所选消息</button><button onClick={() => void exportChosen("txt")}>TXT</button><button onClick={() => void exportChosen("csv")}>CSV</button><button onClick={resetSelection}>清除选择</button></section>}
      {error && <ErrorNotice error={error} retry={() => void Promise.all([loadSources(), loadQueue()])} />}
      {notice && <div className="notice">{notice}</div>}
      <section className="table-card telegram-table-card"><div className="table-meta"><span>{total.toLocaleString()} 条消息</span><div className="button-row"><button className="primary" disabled={busy || !selectedSources.size} onClick={() => void processAllPending()}>一键处理所有未处理</button></div></div><div className="table-scroll desktop-only"><table className="sheet"><thead><tr><th className="check" onClick={() => setSelection(toggleTablePage(selection, pageIds))}><input readOnly type="checkbox" checked={rows.length > 0 && rows.every((row) => isTableRowSelected(selection, row.id))} /></th>{!hidden.has("source") && <th onClick={() => sortBy("source")}>来源</th>}{!hidden.has("date") && <th onClick={() => sortBy("messageDate")}>时间 / ID</th>}{!hidden.has("status") && <th onClick={() => sortBy("status")}>状态</th>}{!hidden.has("body") && <th>消息正文</th>}{!hidden.has("result") && <th>结果</th>}<th>操作</th></tr></thead><tbody>{rows.map((row, index) => <tr key={row.id} className={isTableRowSelected(selection, row.id) ? "selected" : ""} onClick={(event) => selectRow(index, event)} onDoubleClick={() => { setEditRow(row); setEditStatus(row.status === "ignored" ? "ignored" : "pending"); }}><td className="check"><input readOnly disabled={row.status === "deleted"} type="checkbox" checked={isTableRowSelected(selection, row.id)} /></td>{!hidden.has("source") && <td><strong>{row.source_name}</strong></td>}{!hidden.has("date") && <td>{new Date(row.message_date).toLocaleString("zh-CN")}<small className="cell-note">#{row.message_id}{row.event_kind === "edited" ? " · 已编辑" : ""}</small></td>}{!hidden.has("status") && <td><span className="status-chip">{row.status}</span></td>}{!hidden.has("body") && <td className="message-body-cell">{row.remote_deleted_at ? "Telegram 远端消息已删除" : row.body || "正文已按处理策略清理"}</td>}{!hidden.has("result") && <td>{row.candidate_count ? `${row.candidate_count} 条` : "—"}</td>}<td><button onClick={(event) => { event.stopPropagation(); setEditRow(row); setEditStatus(row.status === "ignored" ? "ignored" : "pending"); }}>编辑状态</button></td></tr>)}</tbody></table></div><div className="mobile-telegram-list mobile-only">{rows.map((row, index) => <article key={row.id} className={`mobile-table-card ${isTableRowSelected(selection, row.id) ? "selected" : ""}`} onClick={(event) => selectRow(index, event)}><div className="mobile-card-heading"><input readOnly disabled={row.status === "deleted"} type="checkbox" checked={isTableRowSelected(selection, row.id)} /><span className="status-chip">{row.status}</span><span>{new Date(row.message_date).toLocaleString("zh-CN")}</span></div><strong>{row.source_name} · #{row.message_id}</strong><p>{row.remote_deleted_at ? "Telegram 远端消息已删除" : row.body || "正文已按处理策略清理"}</p><small>{row.candidate_count ? `${row.candidate_count} 条结果` : "尚无结果"}</small><button onClick={(event) => { event.stopPropagation(); setEditRow(row); setEditStatus(row.status === "ignored" ? "ignored" : "pending"); }}>编辑状态</button></article>)}</div>{!rows.length && <div className="empty compact"><strong>当前筛选没有消息</strong><p>选择已绑定来源并刷新同步，或调整状态、关键词和时间范围。</p></div>}<footer className="pagination"><span>第 {page} / {pages} 页</span><select value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1); resetSelection(); }}><option value="20">20 / 页</option><option value="50">50 / 页</option><option value="100">100 / 页</option></select><button disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>上一页</button><button disabled={page >= pages} onClick={() => setPage((value) => value + 1)}>下一页</button></footer></section>
