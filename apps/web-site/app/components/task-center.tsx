...
function taskMetrics(task: Task): Metrics {
  ...
  return {
    ...
    success:
      Number(
        value.success ??
          (normalizeTaskStatus(task.stage) === "completed" ? total : 0),
      ) || 0,
...
function phaseLabel(value: unknown) {
  ...
  if (status === "needs_manual" || status === "partial_completed")
    return "待复查";
  return "已完成";
}
...
function taskMetadata(task: Task) {
  try {
    const value = JSON.parse(task.metadata_json || "{}");
...
export default function TaskCenter({
  ...
  onOpenTask?: (tool: Task["tool"], runId: string) => void;
}) {
  ...
  const filters = useMemo(
    () => ({ phase, stage: status, tool, search }),
    [phase, status, tool, search],
  );
  ...
  const singleTask = useMemo(
    () =>
      selection.mode === "ids" && selection.ids.size === 1
        ? rows.find((row) => selection.ids.has(row.id)) || null
        : null,
    [selection, rows],
  );
    ...
    const loadOutput = async () => {
      ...
      try {
        ...
        const first = await api(
          `/api/runs?id=${encodeURIComponent(task.run_id)}&resultPage=1&resultPageSize=500`,
        );
        const outputRows = [...(first.results || [])] as Array<
          Record<string, unknown>
        >;
        const pageCount = Math.max(
          1,
          Math.ceil(Number(first.total || 0) / 500),
        );
        for (let resultPage = 2; resultPage <= pageCount; resultPage += 1) {
          const part = await api(
            `/api/runs?id=${encodeURIComponent(task.run_id)}&resultPage=${resultPage}&resultPageSize=500`,
          );
        ...
        if (!cancelled) setTaskOutputLoading(false);
      }
    };
  ...
  async function move(next: TaskStatus, target?: Task) {
    ...
    try {
      const result = await api("/api/tasks", {
        ...
        body: JSON.stringify({ ...payload, stage: next }),
      });
      setNotice(
        `已更新 ${Number(result.changed || 0).toLocaleString()} 个任务为“${taskStatusLabel(next)}”${result.snapshotId ? `；恢复点 ${String(result.snapshotId).slice(0, 8)}… 已建立` : ""}`,
      );
      setEditTask(null);
      resetSelection();
      await load();
  ...
  async function cancelSelected() {
    ...
    if (
      !window.confirm(
        `确认取消 ${selectedCount.toLocaleString()} 个任务？原始记录和历史不会删除。`,
      )
    )
      return;
  ...
  function openSelectedTask() {
    if (!onOpenTask || selection.mode !== "ids" || selection.ids.size !== 1)
      return;
    const task = rows.find((row) => selection.ids.has(row.id));
    if (!task?.run_id) {
      setError({
        summary:
          "所选任务还没有可载入的运行 ID，请先完成提取或选择已保存历史对应的任务。",
      });
      return;
    }
  ...
  async function exportSelection(format: "txt" | "csv", copy = false) {
    ...
    try {
      ...
      if (copy) {
        ...
        setNotice(
          `已复制 ${Number(result.count || 0).toLocaleString()} 个任务`,
        );
      } else {
        download(
          `tasks.${format}`,
  ...
  function keyboard(event: React.KeyboardEvent<HTMLDivElement>) {
    ...
    if (modifier && event.key.toLowerCase() === "a") {
      ...
      event.preventDefault();
      setEditTask(rows[focused]);
      setEditStatus(normalizeTaskStatus(rows[focused].stage));
  ...
  return (
    <div className="stack-md" tabIndex={0} onKeyDown={keyboard}>
      <nav className="inbox-stage-tabs" aria-label="处理阶段">
        {PHASES.map((item) => (
          <button
            key={item.phase || "all"}
          ...
          >
            ...
            <small>
              {item.phase
                ? (counts[item.phase] || 0).toLocaleString()
      ...
      <section className="data-toolbar card">
        ...
        <button onClick={() => setShowMetrics((value) => !value)}>
          列设置
        </button>
      ...
      {selectedCount > 0 && (
        <section className="bulkbar">
          <strong>已选 {selectedCount.toLocaleString()} 个</strong>
          {selection.mode === "ids" && selectedCount < total && (
            <button
              onClick={() => setSelection({ mode: "all", excluded: new Set() })}
            >
              全选筛选结果 {total.toLocaleString()} 个
            </button>
          )}
          <select
            defaultValue=""
            onChange={(event) => {
              if (event.target.value)
                void move(event.target.value as TaskStatus);
              event.target.value = "";
            }}
          >
          ...
          <button className="danger" onClick={() => void cancelSelected()}>
            取消任务
          </button>
      ...
      <section className="table-card task-table-card">
          ...
          <table className="sheet task-sheet">
            <thead>
              <tr>
                ...
                <th>ID</th>
                <th onClick={() => sortBy("stage")}>阶段 / 状态</th>
                <th onClick={() => sortBy("tool")}>工具</th>
                <th onClick={() => sortBy("title")}>任务名称</th>
                <th>来源</th>
                <th>摘要</th>
                <th>异常</th>
                <th>运行 ID</th>
                ...
                <th>创建时间</th>
                <th onClick={() => sortBy("updatedAt")}>更新时间</th>
                <th>操作</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row, index) => {
                const metrics = taskMetrics(row);
                const metadata = taskMetadata(row);
                const statusValue = normalizeTaskStatus(row.stage);
                return (
                  <tr
                    key={row.id}
                    className={`${isTableRowSelected(selection, row.id) ? "selected" : ""} ${focused === index ? "focused-row" : ""}`}
                  ...
                  >
                    ...
                    <td>
                      <strong>
                        {phaseLabel(row.resolved_phase || row.phase || row.stage)}
                      </strong>
                      <small className="cell-note">
                        {taskStatusLabel(statusValue)}
                      </small>
                      {row.stage !== statusValue && (
                        <small className="legacy-status">
                          兼容旧值：{row.stage}
                        </small>
                      )}
                    </td>
                    ...
                    <td>
                      {String(
                        metadata.summary ||
                          `${metrics.success}/${metrics.total} 成功`,
                      )}
                    </td>
                    <td>
                    ...
                    {showMetrics && (
                      <>
                        ...
                        <td>
                          {metrics.empty.toLocaleString()} /{" "}
                          {metrics.error.toLocaleString()}
                        </td>
                        <td>
                          {metrics.actualSpeed
                            ? `${metrics.actualSpeed.toFixed(1)}/秒`
                            : "—"}{" "}
                          /{" "}
                          {metrics.etaSeconds
                            ? `${Math.ceil(metrics.etaSeconds / 60)} 分`
                            : "—"}
                        </td>
                      </>
                    )}
                    ...
                    <td>
                      <div className="button-row">
                        {onOpenTask && row.run_id && (
                          <button
                            className="primary"
                            onClick={(event) => {
                              event.stopPropagation();
                              onOpenTask(row.tool, row.run_id);
                            }}
                          >
                            打开
                          </button>
        ...
        <div className="mobile-task-list mobile-only">
          {rows.map((row, index) => {
            const metrics = taskMetrics(row);
            const statusValue = normalizeTaskStatus(row.stage);
            return (
              ...
              >
                <div className="mobile-card-heading">
                  ...
                  <span className={`status-chip task-status-${statusValue}`}>
                    {taskStatusLabel(statusValue)}
                  </span>
                </div>
                <strong>{row.title}</strong>
                <dl>
                  <div>
                    <dt>总量 / 成功</dt>
                    <dd>
                      {metrics.total} / {metrics.success}
                    </dd>
                  </div>
                  <div>
                    <dt>空结果 / 错误</dt>
                    <dd>
                      {metrics.empty} / {metrics.error}
                    </dd>
                  </div>
                  <div>
                    <dt>速度 / ETA</dt>
                  ...
                  <div>
                    ...
                    <dd>
                      {new Date(
                        metrics.lastActivityAt || row.updated_at,
                ...
                {metrics.errorSummary && (
                  <details onClick={(event) => event.stopPropagation()}>
                    <summary>{metrics.errorSummary}</summary>
                    {metrics.errorDetail && <pre>{metrics.errorDetail}</pre>}
                  </details>
                )}
                <div className="button-row">
                  <button
      ...
      {singleTask?.run_id && (
        <section className="stack-md task-output-detail">
          <div className="tool-stage-summary card">
            <div>
              <span className="eyebrow">所选任务内容</span>
              <h3>{singleTask.title}</h3>
              <p className="stage-purpose">
                与对应工具结果页使用相同字段规则；规范链接或番号可直接逐字段复制。
              </p>
            </div>
            <button className="primary" onClick={openSelectedTask}>
              打开完整任务
            </button>
          </div>
          {taskOutputLoading ? (
            <div className="empty compact">
              <strong>正在载入任务结果…</strong>
            </div>
          ) : taskOutput?.taskId === singleTask.id ? (
            <PlainOutputPanel
              tool={taskOutput.tool}
              filtered={taskOutput.rows}
              selected={[]}
              notice={setNotice}
            />
          ) : (
            <div className="empty compact">
              <strong>这个任务还没有正式结果</strong>
              <p>新收到的 Telegram 消息需先在对应工具中处理。</p>
            </div>
          )}
        </section>
      )}
      {editTask && (
        ...
        >
          ...
          >
            <span className="eyebrow">任务状态</span>
            <h3>{editTask.title}</h3>
            <select
              value={editStatus}
              onChange={(event) =>
            ...
            <div className="button-row">
              <button
                className="primary"
                onClick={() => void move(editStatus, editTask)}
              >
                保存
              </button>
