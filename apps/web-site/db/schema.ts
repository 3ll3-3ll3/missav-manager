...
export const contentRuns = sqliteTable(
  ...
  (table) => [
    index("content_runs_tool_created_idx").on(table.tool, table.createdAt),
  ],
);

export const contentResults = sqliteTable(
  "content_results",
  {
    id: text("id").primaryKey(),
    runId: text("run_id")
...
export const permanentRecords = sqliteTable(
  "permanent_records",
  {
    id: text("id").primaryKey(),
    tool: text("tool").notNull(),
  ...
  (table) => [
    uniqueIndex("permanent_records_tool_key_uq").on(
      table.tool,
      table.recordKey,
    ),
    index("permanent_records_tool_status_idx").on(
      table.tool,
      table.status,
      table.updatedAt,
    ),
...
export const importChanges = sqliteTable(
  ...
  (table) => [
    uniqueIndex("import_changes_batch_record_uq").on(
      table.batchId,
      table.tool,
      table.recordKey,
    ),
...
export const importBatchChunks = sqliteTable(
  "import_batch_chunks",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    batchId: text("batch_id")
  ...
  (table) => [
    uniqueIndex("import_batch_chunks_batch_index_uq").on(
      table.batchId,
      table.chunkIndex,
    ),
...
export const inputSources = sqliteTable(
  "input_sources",
  {
    id: text("id").primaryKey(),
    ...
    latestRemoteMessageId: text("latest_remote_message_id")
      .notNull()
      .default(""),
    incrementalCheckpointId: text("incremental_checkpoint_id")
      .notNull()
      .default(""),
  ...
  (table) => [
    uniqueIndex("input_sources_kind_external_uq").on(
      table.kind,
      table.externalKey,
    ),
    uniqueIndex("input_sources_connection_chat_uq").on(
      table.connectionId,
      table.externalChatId,
    ),
    index("input_sources_connection_status_idx").on(
      table.connectionId,
      table.accessStatus,
      table.updatedAt,
    ),
...
export const toolSourceBindings = sqliteTable(
  "tool_source_bindings",
  {
    id: text("id").primaryKey(),
    sourceId: text("source_id")
      .notNull()
      .references(() => inputSources.id, { onDelete: "cascade" }),
    tool: text("tool").notNull(),
  ...
  (table) => [
    uniqueIndex("tool_source_bindings_source_tool_uq").on(
      table.sourceId,
      table.tool,
    ),
  ],
...
export const telegramMessages = sqliteTable(
  ...
  (table) => [
    uniqueIndex("telegram_messages_source_message_uq").on(
      table.sourceId,
      table.messageId,
    ),
    index("telegram_messages_date_idx").on(table.messageDate, table.id),
    index("telegram_messages_source_date_id_idx").on(
      table.sourceId,
      table.messageDate,
      table.id,
    ),
...
export const telegramToolQueue = sqliteTable(
  "telegram_tool_queue",
  {
    id: text("id").primaryKey(),
    telegramMessageId: text("telegram_message_id")
      .notNull()
      .references(() => telegramMessages.id, { onDelete: "cascade" }),
    tool: text("tool").notNull(),
    status: text("status").notNull().default("pending"),
    messageDate: text("message_date").notNull().default(""),
    candidateCount: integer("candidate_count").notNull().default(0),
    candidatePreview: text("candidate_preview").notNull().default(""),
  ...
  (table) => [
    uniqueIndex("telegram_tool_queue_message_tool_uq").on(
      table.telegramMessageId,
      table.tool,
    ),
    index("telegram_tool_queue_tool_status_idx").on(
      table.tool,
      table.status,
      table.updatedAt,
    ),
    index("telegram_tool_queue_message_status_idx").on(
      table.telegramMessageId,
      table.status,
    ),
    index("telegram_tool_queue_tool_status_date_id_idx").on(
      table.tool,
      table.status,
      table.messageDate,
      table.id,
...
export const telegramMessageFingerprints = sqliteTable(
  ...
  (table) => [
    uniqueIndex("telegram_message_fingerprints_source_message_uq").on(
      table.sourceId,
      table.messageId,
    ),
  ],
...
export const telegramConnections = sqliteTable(
  ...
  (table) => [
    index("telegram_connections_kind_idx").on(table.kind, table.status),
  ],
);
...
export const telegramBotState = sqliteTable("telegram_bot_state", {
  connectionId: text("connection_id")
    .primaryKey()
    .references(() => telegramConnections.connectionId, {
      onDelete: "cascade",
    }),
...
export const telegramReadStates = sqliteTable("telegram_read_states", {
  ...
  lastMarkedReadMessageId: text("last_marked_read_message_id")
    .notNull()
    .default(""),
...
export const telegramSyncRuns = sqliteTable(
  "telegram_sync_runs",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    index("telegram_sync_runs_created_idx").on(
      table.startedAt,
      table.connectionId,
    ),
  ],
...
export const telegramAuthFlows = sqliteTable(
  "telegram_auth_flows",
  {
    id: text("id").primaryKey(),
...
export const syncTransactions = sqliteTable(
  "sync_transactions",
  {
    id: text("id").primaryKey(),
...
export const taskInbox = sqliteTable(
  "task_inbox",
  {
    id: text("id").primaryKey(),
    tool: text("tool").notNull(),
    stage: text("stage").notNull(),
    phase: text("phase").notNull().default("received"),
  ...
  (table) => [
    index("task_inbox_stage_updated_idx").on(table.stage, table.updatedAt),
  ],
);

export const scriptGenerations = sqliteTable(
  "script_generations",
  {
    id: text("id").primaryKey(),
    runId: text("run_id").notNull().default(""),
...
export const dataSnapshotItems = sqliteTable(
  "data_snapshot_items",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    snapshotId: text("snapshot_id")
      .notNull()
      .references(() => dataSnapshots.id, { onDelete: "cascade" }),
