...
function sourceKind(
  ...
  if (
    Boolean(entity.broadcast) ||
    (className.includes("channel") && !Boolean(entity.megagroup))
  )
    return "channel";
  if (Boolean(entity.megagroup) || Boolean(dialog.isChannel))
    return "supergroup";
  return "group";
}
...
function credentials() {
  ...
  if (!Number.isInteger(apiId) || apiId <= 0 || !apiHash) {
    throw new Error(
      "请先在 Site Secrets 配置 TELEGRAM_API_ID 和 TELEGRAM_API_HASH",
    );
  }
...
function encryptionSecret() {
  ...
  if (secret.length < 32) {
    throw new Error(
      "请先在 Site Secrets 配置至少 32 字符的 TELEGRAM_SESSION_ENCRYPTION_KEY",
    );
  }
  return secret;
}
...
function accountSummary(user: TelegramUser) {
  ...
  return {
    ...
    accountLabel: (
      name || (user.username ? `@${user.username}` : "Telegram 账号")
    ).slice(0, 160),
  };
}
...
async function flowRow() {
  ...
  return getD1()
    .prepare(
      "SELECT mode,stage,encrypted_session,challenge_json,encrypted_challenge,expires_at FROM telegram_auth_flows WHERE id=?",
    )
    .bind(OWNER_ID)
    .first<FlowRow>();
}
...
async function requireFlow(stages?: FlowRow["stage"][]) {
  ...
  if (stages && !stages.includes(row.stage))
    throw new Error("当前登录阶段不接受此操作");
  return row;
}
...
async function saveFlow(input: {
  ...
  const encrypted = await encryptMtprotoSession(
    input.session,
    encryptionSecret(),
  );
  const encryptedChallenge = input.challenge
    ? await encryptMtprotoSession(
  ...
  await getD1()
    .prepare(
      `INSERT INTO telegram_auth_flows(id,mode,stage,encrypted_session,challenge_json,encrypted_challenge,expires_at,created_at,updated_at)
...
async function deleteFlow() {
  await ensureSchema();
  await getD1()
    .prepare("DELETE FROM telegram_auth_flows WHERE id=?")
    .bind(OWNER_ID)
    .run();
}
...
async function saveAccount(user: TelegramUser, session: string) {
  ...
  await getD1().batch([
    getD1()
      .prepare(
        `INSERT INTO telegram_accounts(id,status,encrypted_session,account_key,account_label,created_at,updated_at)
...
function transportFailure(error: unknown, transport: string) {
  ...
  return Object.assign(
    new Error(String(error || "MTProto transport failure")),
    { mtprotoTransport: transport },
  );
}
...
function tagFailurePhase(error: unknown, phase: string) {
  ...
  return Object.assign(
    new Error(String(error || "MTProto operation failure")),
    { mtprotoPhase: phase },
  );
}
...
function attachClientDiagnostics(
  ...
  const tagged = transportFailure(
    tagFailurePhase(error, phase),
    connection.transport,
  );
  if (tagged && typeof tagged === "object") {
    Object.assign(tagged, {
      mtprotoClientLog: connection.clientLog.slice(-24),
    });
  }
  return tagged;
}
...
async function connectedClient(
  sessionText: string,
  attempt: (typeof TRANSPORT_ATTEMPTS)[number],
) {
  ...
  try {
    ...
    throw attachClientDiagnostics(
      error,
      { transport: attempt.name, clientLog },
      "connect",
    );
  }
}
...
function dialogRows(value: unknown): Array<Record<string, unknown>> {
  ...
  if (value && typeof value === "object") {
    ...
    if (Array.isArray(object.dialogs))
      return object.dialogs as Array<Record<string, unknown>>;
  }
  return [];
}
...
function dialogTitle(
  ...
  return String(
    entity.title ?? dialog.title ?? dialog.name ?? entity.username ?? fallback,
  ).slice(0, 240);
}

function dialogLatestMessageId(dialog: Record<string, unknown>) {
  const message =
    dialog.message && typeof dialog.message === "object"
      ? (dialog.message as Record<string, unknown>)
      : {};
  const value = String(
    message.id ?? dialog.topMessage ?? dialog.top_message ?? "",
  ).trim();
...
export async function discoverPersonalSources() {
  ...
  try {
    ...
    if (!dynamic.getDialogs)
      throw new Error("当前 Telegram 客户端不支持会话发现");
    ...
    const finalStatements = [
      getD1()
        .prepare(
          "INSERT OR IGNORE INTO telegram_read_states(source_id,policy,safe_read_message_id,last_marked_read_message_id,read_baseline_message_id,updated_at) SELECT id,'never','','',latest_remote_message_id,? FROM input_sources WHERE connection_id='telegram-personal'",
        )
        .bind(timestamp),
      getD1()
        .prepare(
          "UPDATE telegram_connections SET status='session_ready',network_status='verified_on_demand',last_success_at=?,last_error='',updated_at=? WHERE connection_id='telegram-personal'",
        )
    ...
    if (!truncated) {
      finalStatements.unshift(
        getD1()
          .prepare(
            "UPDATE input_sources SET access_status='inaccessible',last_error='本次刷新未发现该会话',updated_at=? WHERE connection_id='telegram-personal' AND updated_at<>?",
          )
          .bind(timestamp, timestamp),
      );
    }
    await getD1().batch(finalStatements);
    await writeLog("info", "telegram_mtproto", "个人账号会话库已刷新", {
      discovered,
    });
...
async function resolveEntity(
  ...
  const reference = username
    ? `@${username.replace(/^@/, "")}`
    : String(source.external_chat_id || "");
  return dynamic.getEntity ? dynamic.getEntity(reference) : reference;
}

async function markReadWithClient(
  client: TelegramClient,
  source: Record<string, unknown>,
  maxId: string,
) {
  ...
  if (sourceType === "channel" || sourceType === "supergroup") {
    await client.invoke(
      new Api.channels.ReadHistory({
        channel: entity as Api.TypeEntityLike,
        maxId: Number(maxId),
      }),
    );
  } else {
...
export type PersonalSyncProgress = {
  phase: string;
  label: string;
  current: number;
  total: number;
...
export async function syncPersonalSources(
  input: {
    sourceIds?: string[];
  ...
  if (mode === "range" && input.end && !Number.isFinite(endTime))
    throw new Error("同步结束时间无效");
  if (
    Number.isFinite(startTime) &&
    Number.isFinite(endTime) &&
    startTime > endTime
  )
    throw new Error("同步开始时间不能晚于结束时间");
  const sourceRows = await getD1()
    .prepare(
      `SELECT s.*,r.policy,r.safe_read_message_id,r.last_marked_read_message_id,r.read_baseline_message_id FROM input_sources s LEFT JOIN telegram_read_states r ON r.source_id=s.id WHERE s.id IN (${requested.map(() => "?").join(",")})`,
    )
    .bind(...requested)
    .all();
  const totalSources = Number(sourceRows.results?.length || 0);
  ...
  try {
    const dynamic = dynamicClient(client);
    if (!dynamic.iterMessages)
      throw new Error("当前 Telegram 客户端不支持历史读取");
    const sourceQueue = await runAdaptiveTelegramSourceQueue(
      (sourceRows.results ?? []) as Array<Record<string, unknown>>,
      async (source) => {
        ...
        if (mode === "history") {
          const oldest = await getD1()
            .prepare(
              "SELECT MIN(CAST(message_id AS INTEGER)) AS id FROM telegram_messages WHERE source_id=? AND CAST(message_id AS INTEGER)>0",
            )
            .bind(sourceId)
            .first();
          const beforeId =
            Number(oldest?.id ?? source.sync_cursor_message_id ?? 0) || 0;
          if (beforeId > 0) options.offsetId = beforeId;
        }
        if (mode === "range") {
          if (Number.isFinite(endTime))
            options.offsetDate = Math.floor(endTime / 1_000);
        }
        ...
        const checkpointPlan = telegramSyncCheckpointPlan({
          ...
          latestRemoteMessageId:
            Number(source.latest_remote_message_id || 0) || 0,
        ...
        await getD1().batch([
          ...
          getD1()
            .prepare(
              "UPDATE telegram_sync_runs SET source_id=?,mode=?,has_more=?,checkpoint_before=?,checkpoint_after=? WHERE id=?",
            )
            .bind(
              sourceId,
              mode,
        ...
        if (mode === "incremental") {
          ...
          const markedBefore =
            Number(source.last_marked_read_message_id || 0) || 0;
          await getD1()
            .prepare(
              "UPDATE telegram_read_states SET safe_read_message_id=CASE WHEN ?>CAST(safe_read_message_id AS INTEGER) THEN CAST(? AS TEXT) ELSE safe_read_message_id END,updated_at=? WHERE source_id=?",
            )
            .bind(safeTarget, safeTarget, timestamp, sourceId)
            .run();
          ...
          if (policy === "safe_auto" && safeTarget > markedBefore) {
            try {
              ...
              await getD1()
                .prepare(
                  "UPDATE telegram_sync_runs SET read_result='failed',error_message=? WHERE id=?",
                )
                .bind(
                  "Telegram 远端已读失败；本地安全位置已保留，可重试",
                  commit.syncRunId,
                )
                .run();
              throw new Error(
                "Telegram 远端已读失败；消息和安全位置已保存，可在全局设置中重试。",
              );
            }
            try {
              await getD1()
                .prepare(
                  "UPDATE telegram_read_states SET last_marked_read_message_id=?,updated_at=? WHERE source_id=?",
                )
                .bind(String(safeTarget), nowIso(), sourceId)
                .run();
            } catch {
              await getD1()
                .prepare(
                  "UPDATE telegram_sync_runs SET read_result='external_succeeded_local_state_unknown',error_message=? WHERE id=?",
                )
                .bind(
                  "Telegram 远端已读已发生，但本地状态保存失败；请手动核对",
                  commit.syncRunId,
                )
                .run();
              throw new Error(
                "Telegram 远端已读已成功，但本地状态保存失败；请在全局设置中核对已读状态。",
              );
            }
            await getD1()
              .prepare(
                "UPDATE telegram_sync_runs SET read_result='marked' WHERE id=?",
              )
              .bind(commit.syncRunId)
              .run();
          } else {
            const readResult =
              policy === "never"
                ? "disabled"
                : safeTarget > markedBefore
                  ? "pending_manual"
                  : "already_marked";
            await getD1()
              .prepare("UPDATE telegram_sync_runs SET read_result=? WHERE id=?")
              .bind(readResult, commit.syncRunId)
              .run();
          }
        } else {
          await getD1().batch([
            getD1()
              .prepare(
                "UPDATE telegram_read_states SET read_baseline_message_id=CASE WHEN ?>CAST(read_baseline_message_id AS INTEGER) THEN CAST(? AS TEXT) ELSE read_baseline_message_id END,updated_at=? WHERE source_id=?",
              )
              .bind(maxId, maxId, timestamp, sourceId),
            getD1()
              .prepare(
                "UPDATE telegram_sync_runs SET read_result='history_skipped' WHERE id=?",
              )
              .bind(commit.syncRunId),
          ]);
        }
    ...
    const result = {
      ...totals,
      concurrency: {
        initial: sourceQueue.initialConcurrency,
...
export async function markTelegramSourceRead(
  ...
  const source = await getD1()
    .prepare(
      "SELECT s.*,r.policy,r.safe_read_message_id,r.last_marked_read_message_id FROM input_sources s LEFT JOIN telegram_read_states r ON r.source_id=s.id WHERE s.id=?",
    )
    .bind(sourceId)
    .first<Record<string, unknown>>();
  if (!source) throw new Error("Telegram 来源不存在");
  if (String(source.connection_id || "") !== "telegram-personal")
    throw new Error("只有个人 API 来源支持远端已读");
  if (String(source.policy || "never") === "never")
    throw new Error("当前来源策略为 never；请先改为 manual 或 safe_auto");
  const safeId = String(source.safe_read_message_id || "");
  const requestedId = String(maxIdValue || safeId);
  if (
    !/^\d+$/.test(requestedId) ||
    !/^\d+$/.test(safeId) ||
    BigInt(requestedId) > BigInt(safeId)
  )
  ...
  try {
    ...
    try {
      await getD1()
        ...
        .bind(maxId, nowIso(), sourceId)
        .run();
    } catch {
      throw new Error(
        "Telegram 远端已读已成功，但本地状态保存失败；外部副作用已经发生。",
      );
    }
...
async function safeOperation<T>(
  ...
  try {
    ...
    const detail = {
      ...
      diagnostic: describeMtprotoError(error),
      attempts: Array.isArray(tagged?.mtprotoAttempts)
        ? tagged.mtprotoAttempts
        : [],
      clientLog: Array.isArray(tagged?.mtprotoClientLog)
        ? tagged.mtprotoClientLog
        : [],
    ...
    const loggingNote = logResult.persisted
      ? "脱敏诊断已写入运行日志"
      : "应用日志写入失败，脱敏诊断已写入平台日志";
    throw new Error(
      `${message}（错误码：${failure.code}；追踪 ID：${traceId}；${loggingNote}）`,
    );
  }
}
...
export async function mtprotoStatus() {
  ...
  const [account, flow] = await getD1().batch([
    getD1()
      .prepare(
        "SELECT status,account_label,updated_at FROM telegram_accounts WHERE id=?",
      )
      .bind(OWNER_ID),
    getD1()
      .prepare(
        "SELECT mode,stage,expires_at FROM telegram_auth_flows WHERE id=?",
      )
      .bind(OWNER_ID),
  ]);
  ...
  const expired =
    flowValue && Date.parse(String(flowValue.expires_at)) <= Date.now();
  ...
  return {
    apiConfigured:
      ...
      Boolean(String(env.TELEGRAM_API_HASH || "").trim()),
    encryptionConfigured:
      String(env.TELEGRAM_SESSION_ENCRYPTION_KEY || "").length >= 32,
...
export async function startPhoneLogin(phoneValue: unknown) {
  ...
  await getD1()
    .prepare(
      "INSERT OR IGNORE INTO telegram_connections(connection_id,kind,label,status,created_at,updated_at) VALUES ('telegram-personal','personal','Telegram 个人账号','disconnected',?,?)",
    )
    .bind(nowIso(), nowIso())
    .run();
  credentials();
    ...
    async () => {
      ...
      try {
        ...
        if (sent.emailRequired || sent.emailCodeSent) {
          throw new Error(
            "Telegram 要求邮箱验证；当前最小登录测试尚未支持该额外挑战",
          );
        }
        await saveFlow({
          mode: "phone",
        ...
        await writeLog("info", "telegram_mtproto", "个人账号验证码已请求", {
          stage: "waiting_code",
          delivery: sent.isCodeViaApp ? "app" : "sms",
        });
        return {
          stage: "waiting_code" as const,
          delivery: sent.isCodeViaApp ? "app" : "sms",
          expiresAt: expiresAt(),
        };
      } catch (error) {
    ...
    async () => {
      ...
      try {
        ...
        if (!(result instanceof Api.auth.Authorization))
          throw new Error("Telegram 返回了未知登录状态");
  ...
  try {
    ...
    if (result instanceof Api.auth.LoginTokenSuccess) {
      if (!(result.authorization instanceof Api.auth.Authorization))
        throw new Error("Telegram 返回了未知二维码登录状态");
      return saveAccount(
        result.authorization.user as TelegramUser,
        session.save(),
      );
    }
    if (!(result instanceof Api.auth.LoginToken))
      throw new Error("Telegram 返回了未知二维码状态");
    await saveFlow({
      mode: "qr",
      stage: "waiting_qr",
      session: session.save(),
...
export async function cancelMtprotoLogin() {
  await deleteFlow();
  await writeLog("info", "telegram_mtproto", "个人账号登录流程已取消并解锁", {
    stage: "idle",
  });
...
export async function logoutMtproto() {
  ...
  await getD1().batch([
    ...
    getD1()
      .prepare(
        "UPDATE telegram_connections SET status='disconnected',session_encrypted='',account_key='',account_label='',username='',network_status='unknown',updated_at=? WHERE connection_id='telegram-personal'",
      )
      .bind(nowIso()),
  ]);
  await writeLog("info", "telegram_mtproto", "网站端个人账号 Session 已删除", {
    remoteLoggedOut,
  });
