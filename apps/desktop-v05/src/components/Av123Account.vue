...
async function runExtension() {
  if (!candidates.value.length || running.value) return;
  if (!bridge.value?.available) { error.value = bridge.value?.error || "Chrome 扩展桥当前不可用"; return; }
  running.value = true; done.value = 0; total.value = candidates.value.length; error.value = "";
  try {
...
async function exportTasks() {
  ...
  const rows = ["code,url", ...candidates.value.map((item) => `"${item.primaryValue}","${item.secondaryValue}"`)]; await writeTextFile(path, `\uFEFF${rows.join("\r\n")}`); status.value = `已导出 ${candidates.value.length} 条。`;
}
const internalCurrent = computed(() => candidates.value[internalIndex.value] || null);
