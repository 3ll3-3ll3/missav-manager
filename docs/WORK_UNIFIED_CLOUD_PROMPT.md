...
8. 三轮部署前审计发现的七项阻断必须以最新 HEAD 复核：
   ...
   - Bot 必须在消息分块已经写入后、最终 offset 批次提交前再次验租约；失效时保留幂等消息、offset 不前进、同步运行记失败，不能只测试“入库前失效”。
   - Web Push/Pull 必须以有界请求返回明确的 `incomplete`/`hasMore`/`remaining`，网页重新预览并续跑；十万条不能受 500 页硬上限静默截断，分段未完成不能更新最近成功时间或显示同步完成。
   - Pull 第一页成功而后续页网络失败时，只保留已提交业务数据和 `last_pulled_sequence` 断点；`last_success_at` 必须保持执行前原值并写入 `last_error`。逐页应用函数不得更新最近成功时间。
