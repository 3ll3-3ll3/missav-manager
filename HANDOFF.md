...
- **v30 已通过全部候选门禁，可以部署为只读备份中心过渡版；当前生产因下述自动化身份验收问题已回退 v26。** 重新部署仍仅允许同一 v30，不执行 `0006`、不迁移或修改正式 D1、不启动同步。部署后先做 owner 页面、25 表清单、空/小规模备份、中断续传、下载与文件复校 E2E；真实 owner 页面出现异常才回退 v26。
