...
- GitHub PR #4 仍是旧的同名网站分支，核对时 HEAD 为 `f929ac6`，未包含本轮基于生产 v26 的修改。
...
- 123AV 只创建本地番号任务；任务 CSV 固定 `code,url,status,task_id`，支持只导出异常/待核验范围。结果 CSV 先预览匹配、未匹配、重复、坏行及更新量，再确认事务写入。
...
npm run lint          # 通过
npx tsc --noEmit      # 通过
npm test              # 通过，62/62；命令内先完成生产构建和 Sites 产物校验
npm run build         # 通过
cd ../..
git diff --check      # 通过
```
...
- Bad.news 输入混合首页、`/app`、噪声、重复及两条主题链接，只得到两条按首次出现顺序排列的规范链接；复制当前筛选实际得到两行。
- 推特的博主名与主页分别输出；MissAV 显示完整脚本文本；123AV 显示本地任务交接和稳定 CSV 字段。
- 工具切换及页面刷新后，已保存 Bad.news 任务与结果仍能恢复。
- 处理中心可打开该任务并显示相同字段级纯文本；控制台无警告或错误。
...
1. 一次已保存运行的结果页仍会将该运行结果载入浏览器；完整的服务端筛选、排序和“筛选条件 + 排除 ID”选择模型尚未覆盖所有工具结果/任务详情路径。
...
- 本轮没有 D1 schema 迁移，也未读写正式 D1 数据；D1 与 Windows SQLite 继续独立。
