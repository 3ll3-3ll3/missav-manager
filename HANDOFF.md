...
## 当前候选：Site v29 多请求可续传数据库备份中心（未部署）
...
- v27、v28 均未获部署批准。v29 的生产能力仍严格收敛为三项：轻量只读表清单、由所有者显式触发的完整备份下载、备份文件只读校验。生产恢复按钮、生产 `POST restore` API、`DELETE + 分批恢复` 和旧单请求全库下载入口均保持删除或禁用。
...
- 浏览器把已完成的 NDJSON 分块、表号、主键游标、页号和 SHA-256 链状态提交到 IndexedDB；每个完整响应后原子保存。页面关闭、网络中断或手动暂停后可从最后完整页继续。Worker 不向正式 D1 写备份任务、游标、临时行或其他状态。
...
- Sites Agent Preview 使用实际候选 Worker/UI 与本地 Miniflare D1 验收：清单返回 25 表、1 行、2 次查询；在第 19 个请求后暂停并刷新页面，IndexedDB 恢复同一表/游标后完成双遍复核；含一次中断重试和一次完整文件回读校验共 54 个浏览器请求，单请求最高 2 次查询，25 表/1 行文件校验 SHA-256 为 `71244bda3a4608ce57f6fd98f3352fe997b52c8a1a32bea37913536d1307301e`。下载按钮已触发，但云浏览器未暴露程序化 Blob 下载事件；同一 IndexedDB 完整文件已回传同一 owner-only Site 校验通过，不把浏览器抓取限制误报为已取得外部下载文件。
