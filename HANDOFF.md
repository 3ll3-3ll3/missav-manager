...
- v27 未获部署批准。v28 修订候选的生产能力严格收敛为三项：轻量只读表清单、由所有者显式触发的完整备份下载、备份文件只读校验。生产恢复按钮、生产 `POST restore` API、`DELETE + 分批恢复` 实现均已删除。
- 页面加载只查询 25 表 Schema 与 `COUNT(*)`，不自动读取业务行、不计算全库摘要。完整备份使用 250 行有界分页 NDJSON 流，不把全库一次装入 Worker 内存。
- 下载文件逐页形成 SHA-256 链，完成前对 25 表执行第二遍有界扫描；两遍不一致时写入 `complete=false`，校验器拒绝该文件。只有 `complete=true`、逐表摘要和总清单摘要全部通过才算有效备份。
...
- 隔离测试覆盖：中途失败零正式库写入、备份期间数据变化、未获准请求、全新临时 D1 恢复演练，以及 Workers/Miniflare D1 10 万行流式备份与校验。正式 D1 没有参与任何写入测试。
