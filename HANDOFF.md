...
- 当前本地 HEAD：以本文件所在提交为准；交接更新后必须推送 `codex/unified-local-cloud-v1` 并确认本地/远端一致。
...
- 正式同步 Worker/D1 已部署，Worker `0001_initial.sql` 已执行，健康接口和设备协议已由 Windows 隔离 E2E 验证；生产 Site 已部署 v30 只读备份中心，但尚未接入统一同步代码与 `0006` 数据表，因此正式网站和 Windows 仍不能业务互通。
- `0006_spicy_omega_sentinel.sql` 尚未在正式 D1 执行，未导入或改写任何正式数据。
- 真实网站↔Windows 首次汇合、断网续传、十万条正式数据与 Telegram 登录/收发 E2E 尚未验收。
- Site 的 `SYNC_GATEWAY_URL` 与遮蔽的 `SYNC_ADMIN_TOKEN` 环境项已经设置，但当前生产 v30 仍只是只读备份过渡版；不能据此描述成“网站同步已上线”。
- 云端普通 Work 对话若没有挂载当前正式 Site，不能修改、部署或回退生产；应从 Sites 列表点击当前站点的编辑图标进入带 Site 引用的对话。生产 v30 的 Git/Sites/产物对应关系已固定，不得从其他旧 SHA 猜测并覆盖。

### 下一步交给云端

云端下一步必须从 Sites 列表点击当前 TG 内容工具箱的编辑图标，在自动挂载正式 Site 的对话中执行；不得在普通 Work 对话里猜测生产源码：

1. 当前正式生产已确认并部署为 v30；不得重新构建、覆盖或回退，除非真实 owner 页面出现故障。
2. 先等待所有者本人在已登录 Site 中完成生产 E2E：25 表清单、正式只读备份、刷新/中断续传、下载和文件复校；自动化无法模拟 owner 身份不算失败。
3. E2E 期间不得执行 `0006_spicy_omega_sentinel.sql`、写入或迁移正式 D1、启动同步或修改 Secret；控制面继续核对 D1 仍为 25 表且无 `cloud_sync_*`。
4. 所有者 E2E 通过后，把备份文件的行数、SHA-256、完成时间和 Time Travel 可用状态写入交接，但不得提交备份文件或任何正式数据。
5. 下一轮才允许以 v30 对应 Git 基线为生产起点，合并统一同步代码、重新完成候选审计，并在正式备份/Time Travel 门槛满足后单独申请执行 `0006`。
...
- 生产 Site v30 的 Git 提交、tree、Sites 源码提交和产物哈希已经固定并验证对应；不得再使用 v26 或其他旧分支覆盖生产。
- 正式 Site D1 尚未执行 `0006`。执行前仍缺所有者本人完成的生产只读备份、刷新续传、下载与文件复校 E2E，以及可恢复性记录。
- 所有者 E2E 通过后，才允许合并统一同步代码、执行新一轮候选审计并单独申请 `0006`；首次正式汇合仍只生成预览并等待用户确认。
...
### 2026-08-24 Site v29 备份中心候选审计

- 云端候选 Git 提交为 `2ccedc8753822fb78d996c25467362881617ba61`，源码树 `7ca1ba8e29b8c9eef5c9e0c0b8dd547e4a1b1ecc`；Sites v29 已保存但未部署，正式生产仍为 v26。
- Windows 隔离 worktree 已复跑生产构建与全部测试，`62/62` 通过；候选产物仅包含 `0000`～`0004`，未包含 `0006` 或 `cloud_sync_*`。
- v28 的 D1 单请求预算问题已关闭：清单 2 次查询，每个导出/复核分页 2 次，finalize 和文件校验各1次；10 万行由 448 个分页请求完成，没有使用大 `OFFSET`。
- **v29 仍暂不批准部署。** 浏览器续传代码在分页响应后直接修改当前 job 的游标、页号、行数、摘要和序号，然后才在一个 IndexedDB 事务中写 job + chunk。若该事务因配额、存储或浏览器中断失败，catch 会再单独保存已经前进的 job，但缺少对应 chunk。下次续传会跳过该页，导致文件静默缺行。
- 必须把续传改为 copy-on-write 检查点：保留上一个已持久化 job，用独立 `nextJob` 计算下一状态，只有 job + chunks 同一 IndexedDB 事务成功后才切换当前状态。事务失败时必须从 IndexedDB 重读旧检查点，不得保存已前进的内存对象。finalize/footer 也要使用同样语义。
- 增加真实 fake-indexeddb/浏览器回归：模拟 chunk 事务在网络成功后失败，验证 job 游标不前进、重试后该页只出现一次、最终 25 表行数/SHA-256 正确；模拟 footer 事务失败时不得将 job 标记为 complete。
...
- 云端候选 Git 提交为 `38d6c5095e2f75551fa9038f342a3b83ea44f636`，源码树 `40130b380039d7a8028be750be18008a3cdaf275`；Sites v30 当前已正式部署。
...
- 网页 Work 已完成最后只读核验：Site v30 内部源码提交为 `814635569a05e84d66a4596df582b3f9afda029a`，产物 SHA-256 为 `c4dacc484bbb6d2673a5a8c685828387de186552d7fecdde67a288684903ce0b`，并确认与上述 Git 提交及源码树对应。访问模式为 `custom`，唯一 owner 1 人，编辑者/查看者/外部访客/共享群组/Workspace 群组/Tenant 群组均为 0。
- **v30 已通过全部候选门禁。** 首次部署曾因下述自动化身份验收问题回退 v26，随后同一 v30 已重新部署并保持生产；仍不执行 `0006`、不迁移或修改正式 D1、不启动同步。下一步由真实 owner 完成页面、25 表清单、正式备份、中断续传、下载与文件复校 E2E；真实 owner 页面出现异常才回退 v26。
- 所有者随后曾明确授权并成功部署 v30，但网页 Work 的自动化生产请求无法取得 ChatGPT Site 所有者身份上下文，因此无法代替真实 owner 验证受保护接口；它按原指令把“无法证明”判为异常并立即成功回退 v26。控制面权限仍为唯一 owner，匿名请求正确返回 401，正式 D1 仍完整 25 表且无 `cloud_sync_*`，期间没有调用写接口、执行 `0006`、修改 Secret 或启动同步。
- 该结果属于自动化身份能力缺口，不是已经证实的 v30 代码故障。下一次应重新部署同一 v30 后由所有者本人在已登录的 Site 页面完成 owner E2E；自动化继续负责匿名 401、访问策略和 D1 只读核对。只有 owner 页面也失败时才回退并进入代码诊断；不能再仅因自动化无法模拟 owner 而回退。
- 同一 v30 已重新部署成功：生产版本为 v30，访问策略仍为唯一 owner，匿名备份接口返回 401；控制面确认正式 D1 保持完整 25 表、无 `cloud_sync_*`。本次部署后只执行控制面读取和匿名 GET，没有 D1 写入、`0006`、Secret 修改或同步操作。当前唯一待办是所有者本人登录后的备份 E2E。
- 已知非阻断文案问题：v30 备份页顶部仍静态显示“Site v30 备份中心候选（未部署）”，与当前生产状态不符，但不影响 API、D1 只读边界或备份流程。owner E2E 不应因此回退；下一版必须改为实际部署状态或去掉静态部署声明。
