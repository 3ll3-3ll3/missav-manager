...
test('tool registry exposes five isolated tools with stable capabilities', () => {
  const tools = registry.listTools();
  assert.deepEqual(tools.map(tool => tool.id), ['twitter', 'badnews', 'haijiao', 'missav', 'av123']);
  assert.equal(new Set(tools.map(tool => tool.id)).size, tools.length);
  assert.equal(registry.getTool('twitter').capabilities.persistentResults, true);
  assert.equal(registry.getTool('badnews').capabilities.network, false);
  assert.equal(registry.getTool('haijiao').capabilities.persistentResults, true);
...
test('tool registry groups tools for the dedicated home page', () => {
  ...
  assert.deepEqual(groups[0].tools.map(tool => tool.id), ['twitter', 'badnews', 'haijiao']);
